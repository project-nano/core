...
func (manager *ResourceManager) handleCommand(cmd resourceCommand) {
	...
	case cmdRemoveImageServer:
		err = manager.handleRemoveImageServer(cmd.Name)
...
func (manager *ResourceManager) handleSearchGuests(condition SearchGuestsCondition, respChan chan ResourceResult) (err error) {
	...
	var cellSpecified = "" != condition.Cell
	var keywordSpecified = "" != condition.Keyword
	var requestUser = condition.Owner
	var requestGroup = condition.Group
	var targets []string
	...
	if cellSpecified {
		...
		for _, instanceID := range pool.InstanceNames {
			targets = append(targets, instanceID)
		}
	}
	...
	if keywordSpecified {
		...
		if cellSpecified || poolSpecified {
			for _, instanceID := range targets {
				if matched, guestName, err = manager.matchKeyword(instanceID, requestUser, requestGroup,
		...
		targets = filtered
	} else if !cellSpecified && !poolSpecified {
		//fill all instances
		for instanceID, instance := range manager.instances {
			if instance.IsVisible(requestUser, requestGroup) {
				targets = append(targets, instanceID)
		...
		for _, instanceID := range targets {
			if instance, exists := manager.instances[instanceID]; !exists {
				...
				return
			} else if instance.IsVisible(requestUser, requestGroup) {
				idMap[instance.Name] = instanceID
			}
		}
	}
...
func (manager *ResourceManager) matchKeyword(instanceID, requestUser, requestGroup, keyword string) (
	matched bool, name string, err error) {
	matched = false
