...
func (executor *CreateComputePoolExecutor)Execute(id framework.SessionID, request framework.Message,
	...
	if "" == storagePool{
		log.Printf("[%08X] request create compute pool '%s' using local storage from %s.[%08X]", id, pool, request.GetSender(), request.GetFromSession())
	}else{
		log.Printf("[%08X] request create compute pool '%s' using storage pool '%s' from %s.[%08X]", id, pool, storagePool,
	...
	resp, _ := framework.CreateJsonMessage(framework.CreateComputePoolResponse)
	...
	executor.ResourceModule.CreatePool(pool, storagePool, addressPool, failover, respChan)
	err = <-respChan
	if err != nil{
		resp.SetError(err.Error())
		log.Printf("[%08X] create compute pool fail: %s", id, err.Error())
	}else{
