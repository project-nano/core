...
func (executor *CreateMediaImageExecutor) Execute(id framework.SessionID, request framework.Message,
	...
	if err = executor.Sender.SendMessage(request, imageServer); err != nil {
		log.Printf("[%08X] forward create media to image server fail: %s", id, err.Error())
		resp.SetError(err.Error())
		return executor.Sender.SendMessage(resp, request.GetSender())
	}
	//wait response
	...
	case forwardResp := <-incoming:
		if !forwardResp.IsSuccess() {
			log.Printf("[%08X] create media image fail: %s", id, forwardResp.GetError())
		}
	...
	case <-timer.C:
		//timeout
		log.Printf("[%08X] create media image timeout", id)
