...
func (executor *EjectMediaExecutor)Execute(id framework.SessionID, request framework.Message,
	...
	log.Printf("[%08X] request eject media from guest '%s' from %s.[%08X]", id, instanceID,
	...
	{
		//forward request
		forward, _ := framework.CreateJsonMessage(framework.EjectMediaRequest)
		...
		if err = executor.Sender.SendMessage(forward, ins.Cell); err != nil{
			log.Printf("[%08X] forward eject to cell '%s' fail: %s", id, ins.Cell, err.Error())
			resp.SetError(err.Error())
			return executor.Sender.SendMessage(resp, request.GetSender())
		}
		...
		case cellResp := <- incoming:
			if cellResp.IsSuccess(){
				log.Printf("[%08X] cell eject media success", id)
			}else{
				log.Printf("[%08X] cell eject media fail: %s", id, cellResp.GetError())
			}
		...
		case <- timer.C:
			//timeout
			log.Printf("[%08X] wait eject response timeout", id)
