...
type MoveGuestSecurityRuleExecutor struct {
	Sender         framework.MessageSender
	ResourceModule modules.ResourceModule
}

func (executor *MoveGuestSecurityRuleExecutor)Execute(id framework.SessionID, request framework.Message,
	...
	resp.SetSuccess(false)
	{
		...
		if result.Error != nil{
			err = result.Error
			log.Printf("[%08X] get instance '%s' for move security rule fail: %s",
	...
	}
	{
		...
		if err = executor.Sender.SendMessage(forward, instance.Cell); err != nil{
			log.Printf("[%08X] forward move security rule to cell '%s' fail: %s", id, instance.Cell, err.Error())
			resp.SetError(err.Error())
			return executor.Sender.SendMessage(resp, request.GetSender())
		}
		...
		case cellResp := <- incoming:
			if cellResp.IsSuccess(){
				log.Printf("[%08X] %dth security rule of instance '%s' moved",
					id, index, instance.Name)
			}else{
				log.Printf("[%08X] cell move security rule fail: %s", id, cellResp.GetError())
			}
		...
		case <- timer.C:
			//timeout
			log.Printf("[%08X] wait move security rule response timeout", id)
